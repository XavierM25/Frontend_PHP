document.addEventListener("DOMContentLoaded", function () {
        ...
        if (data.status === 200 && Array.isArray(data.videos)) {
          ...
          if (currentSubcategoryId) {
            videos = videos.filter(
              (video) => video.subcategoria_id === currentSubcategoryId
            );
          }
    ...
    fetch("http://localhost/frontend_php/api/proxy/ranking", {
      ...
      .then((data) => {
        ...
        if (
          ...
          noRankingsMessage.textContent =
            "No hay rankings disponibles actualmente.";
  ...
  function createUserRanking(ranking) {
    ...
    h4.textContent = `${ranking.usuario.apellido_paterno || ""} ${
