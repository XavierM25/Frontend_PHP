document.addEventListener("DOMContentLoaded", function () {
  ...
  const vacancySubcategoryInput = document.getElementById(
    "vacancySubCategorie"
  );
    ...
    fetch("https://backend-laravel-wl09.onrender.com/api/categories", {
      ...
      .then((data) => {
        if (
          ...
          console.error(
            "No se encontraron categorías válidas o la lista está vacía"
          );
        }
      })
    ...
    fetch(
      ...
      .then((data) => {
        subcategorySelect.innerHTML =
          '<option value="" disabled selected>Subcategoría</option>'; // Limpiar opciones anteriores
    ...
    fetch(
      ...
      .then((data) => {
        if (data.totalVacantes !== undefined) {
          ...
          const occupiedVacanciesSpan =
            document.getElementById("occupiedVacancies");
          const availableVacanciesSpan =
            document.getElementById("availableVacancies");
  ...
  function validateAndEnableCheckboxes(horarios) {
    ...
    horarios.forEach((horario) => {
      const dia =
        ...
        horario.dia.slice(1).toLowerCase(); // Convertir el día a formato capitalizado
      const checkbox = document.querySelector(
        `input[name="dias[]"][value="${dia}"]`
      );
      if (checkbox) {
  ...
  function updateSchedulesSelect(horarios) {
    scheduleSelect.innerHTML =
      '<option value="" disabled selected>Selecciona un horario</option>'; // Limpiar opciones anteriores
  ...
  form.addEventListener("submit", function (event) {
    ...
    fetch("https://backend-laravel-wl09.onrender.com/api/inscripciones", {
      method: "POST",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
