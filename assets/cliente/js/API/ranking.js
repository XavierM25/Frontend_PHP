document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchAndShowCategories() {
    fetch("https://backend-laravel-wl09.onrender.com/api/categories", {
      ...
      .then((data) => {
        filterRankSection.innerHTML =
          '<li class="active" id="all-categories">Todo</li>';

        const allCategoriesItem = document.getElementById("all-categories");
        allCategoriesItem.addEventListener("click", () => {
          document
            .querySelectorAll(".categories-rank li")
      ...
      .then((data) => {
        ...
        data.subcategorias.forEach((subcategoria) => {
          ...
          subcategoryItem.addEventListener("click", () => {
            document
              .querySelectorAll(".subcategories-rank li")
              .forEach((li) => li.classList.remove("active"));
        ...
        if (data.status === 200 && Array.isArray(data.rankings)) {
          ...
          if (currentSubcategoryId) {
            rankings = rankings.filter(
              (ranking) => ranking.subcategoria_id === currentSubcategoryId
            );
          }
  ...
  function updateTop3Rankings(rankings) {
    ...
    rankings.forEach((ranking, index) => {
      ...
      rankHeader.innerHTML = `#${index + 1} <p>${
        ranking.subcategoria ? ranking.subcategoria.nombre : ""
      }</p>`;
      ...
      userName.textContent = `${ranking.usuario.apellido_paterno || ""} ${
        ranking.usuario.apellido_materno || ""
      } ${ranking.usuario.nombre || ""}`;
    ...
    if (isTableVisible) {
      ...
      rankings.forEach((ranking, index) => {
        ...
        row.innerHTML = `
                    ...
                    <td>${ranking.usuario.apellido_paterno || ""} ${
          ranking.usuario.apellido_materno || ""
        }</td>
                    <td>${
                      ranking.subcategoria
                        ? `${
                            ranking.subcategoria.categoria
                              ? ranking.subcategoria.categoria.nombre
                              : ""
                          } | ${ranking.subcategoria.nombre}`
                        : ""
                    }</td>
  ...
  function showStudentDetails(ranking) {
    studetPosition.textContent =
      ranking.posicion !== undefined && ranking.posicion !== null
        ? `#${ranking.posicion}`
        : "";
    studentCategorySubca.textContent = ranking.subcategoria
      ? `${
          ranking.subcategoria.categoria
            ? ranking.subcategoria.categoria.nombre
            : ""
    ...
    studentFullName.textContent = `${ranking.usuario.nombre || ""} ${
      ranking.usuario.apellido_paterno || ""
    } ${ranking.usuario.apellido_materno || ""}`;
    studentCareer.textContent = ranking.usuario.carrera || "";
    detailCat.value = ranking.subcategoria
      ? ranking.subcategoria.categoria
        ? ranking.subcategoria.categoria.nombre
        : ""
      : "";
    detailSubCat.value = ranking.subcategoria
      ? ranking.subcategoria.nombre
      : "";
