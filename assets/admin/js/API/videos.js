document.addEventListener("DOMContentLoaded", function () {
  const baseUrl = "https://backend-laravel-wl09.onrender.com/api/videos";
  ...
  function handleCrearVideoClick(event) {
    ...
    if (!descripcion.trim() || !titulo.trim() || !videoFile) {
      alert(
        "Debe ingresar una descripción, un título y seleccionar un archivo de video."
      );
      return;
    }
      ...
      .then((data) => {
        ...
        data.videos.forEach((video) => {
          ...
          row.innerHTML = `
                    ...
                    <td>
                        <div class="editCont">
                            <button class="btnEditarVideo" data-id="${
                              video.id
                            }"><i class="ri-pencil-fill"></i></button>
                        </div>
                        <div class="deleteCont">
                            <button class="btnEliminarVideo" data-id="${
                              video.id
  ...
  function showVideoDetails(video) {
    lastVideoUploaded.src = `https://backend-laravel-wl09.onrender.com/${video.archivo_video}`;
  ...
  function openEditModal(videoId) {
    fetch(`${baseUrl}/${videoId}`, {
      ...
      .then((video) => {
        ...
        loadEditCategories(
          video.categoria ? video.categoria.id : null,
          video.subcategoria ? video.subcategoria.id : null
        );
  ...
  cerrarEliminarVideo.addEventListener("click", () =>
    closeModal(modalConEliminarVideo, modalEliminarVideo)
  );
