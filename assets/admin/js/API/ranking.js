document.addEventListener("DOMContentLoaded", function () {
  ...
  function deleteRanking(usuarioId) {
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/ranking/${usuarioId}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  function showUserDetails(ranking) {
    ...
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/users/${ranking.usuario.id}/habilities-with-position`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          document.getElementById(
            "nameRnk"
          ).value = `${ranking.usuario.nombre} ${ranking.usuario.apellido_paterno}`;
          ...
          if (!currentSubcategoryId) {
            document.getElementById("positionRnk").value =
              posicionGeneral !== undefined ? posicionGeneral : "";
          } else {
            // Mostrar posición en la subcategoría si hay filtro
            document.getElementById("positionRnk").value =
              posicionSubcategoria !== null ? posicionSubcategoria : "";
          }
    ...
    fetch(
      ...
      .then((data) => {
        if (
          ...
          console.error(
            "No se recibió un arreglo válido de subcategorías:",
            data
          );
          return;
        }
