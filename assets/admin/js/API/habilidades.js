document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadUsersAndSkills() {
    fetch("https://backend-laravel-wl09.onrender.com/api/users", {
      method: "GET",
      headers: {
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          data.users.forEach((user) => {
            ...
            fetch(
              `https://backend-laravel-wl09.onrender.com/api/users/${user.id}/habilities`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
              ...
              .then((habilitiesData) => {
                if (
                  habilitiesData.status === 200 &&
                  habilitiesData.habilidades.length > 0
                ) {
                  const habilidad = habilitiesData.habilidades[0];
                  row.querySelector("td:nth-child(4)").textContent =
                    habilidad.categoria.nombre;
                  row.querySelector("td:nth-child(5)").textContent =
                    habilidad.subcategoria.nombre;
                  ...
                  row
                    .querySelector('input[type="checkbox"]')
                    .setAttribute("data-habilidad-id", habilidad.id);
                } else {
  ...
  function loadSkills(userId) {
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/users/${userId}/habilities`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        ...
        if (
          ...
          loadCategories(() => {
            document.getElementById("category-select").value =
              habilidad.categoria_id;
            loadSubcategories(
              habilidad.categoria_id,
              habilidad.subcategoria_id
            );
          });
        } else {
  ...
  assignBtn.addEventListener("click", function () {
    ...
    fetch("https://backend-laravel-wl09.onrender.com/api/habilities", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
  ...
  editBtn.addEventListener("click", function () {
    ...
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/habilities/${habilidadId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  resetBtn.addEventListener("click", function () {
    ...
    const confirmDelete = confirm(
      "¿Está seguro de eliminar la habilidad seleccionada?"
    );
    ...
    if (confirmDelete) {
      ...
      if (!habilidadId) {
        alert(
          "No se encontró ninguna habilidad asociada al usuario seleccionado."
        );
        return;
      }
      ...
      fetch(
        ...
        .then((response) => {
          if (response.ok) {
            ...
            throw new Error(
              "Error al eliminar la habilidad. Código de estado: " +
                response.status
            );
          }
        })
        .catch((error) => {
          ...
          alert(
            "Error al eliminar la habilidad. Consulte la consola para más detalles."
          );
        });
    }
  });
  ...
  function getUserIdFromSelectedCheckbox() {
    const selectedCheckbox = document.querySelector(
      '#usuarios-table input[type="checkbox"]:checked'
    );
    if (selectedCheckbox) {
  ...
  function getSelectedHabilidadId(userId) {
    const selectedCheckbox = document.querySelector(
      `#usuarios-table input[type="checkbox"][data-user-id="${userId}"]:checked`
    );
    if (selectedCheckbox) {
