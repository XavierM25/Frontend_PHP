document.addEventListener("DOMContentLoaded", function () {
  const baseUrl = "https://backend-laravel-wl09.onrender.com/api";
  ...
  const newUserSection = document.querySelector(
    ".new-user .image-user .slides"
  );
  const leftControl = document.querySelector(
    ".new-user .image-user .controls .left"
  );
  const rightControl = document.querySelector(
    ".new-user .image-user .controls .right"
  );
    ...
    fetch(`${baseUrl}/inscripciones`, {
      ...
      .then((data) => {
        if (data && data.inscripciones) {
          ...
          const approvedInscripciones = data.inscripciones.filter(
            (inscripcion) => inscripcion.estado.toLowerCase() === "aprobado"
          ).length;
  ...
  function updateNewUserSlider(users) {
    if (users.length === 0) {
      newUserSection.innerHTML =
        "<p>No hay nuevos usuarios en las últimas 24 horas.</p>";
      return;
    }
    ...
    users.forEach((user, index) => {
      ...
      slide.innerHTML = `
                <div class="user-img">
                    <img src="https://backend-laravel-wl09.onrender.com/${
                      user.imagen_perfil
                    }" alt="${user.nombre}">
                </div>
                <div class="user-info">
  ...
  function initSlider() {
    const slides = document.querySelectorAll(
      ".new-user .image-user .slides .slide"
    );
    let currentSlide = 0;
  ...
  function fetchRecentActivity() {
      ...
      .then((data) => {
        if (data.status === 200) {
          recentVideos = data.videos
            .filter((video) => new Date(video.created_at) >= twentyFourHoursAgo)
            .map((video, index) => ({
              type: "video",
              ...video,
    ...
    const fetchInscripciones = fetch(`${baseUrl}/inscripciones`, {
      ...
      .then((data) => {
        if (data && data.inscripciones) {
          recentInscripciones = data.inscripciones
            .filter(
              (inscripcion) =>
                new Date(inscripcion.created_at) >= twentyFourHoursAgo
            )
          ...
          console.error(
            "Error al obtener inscripciones recientes:",
            data.message
          );
        }
      })
    ...
    Promise.all([fetchVideos, fetchInscripciones]).then(() => {
      ...
      while (
        ...
        (videosIndex < recentVideos.length ||
          inscripcionesIndex < recentInscripciones.length)
      ) {
