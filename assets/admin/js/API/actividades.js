document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadSubcategories(categoriaId) {
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/categories/${categoriaId}/subcategories`,
      {
        method: "GET",
        headers: {
      ...
      .then((data) => {
        ...
        if (data.subcategorias && Array.isArray(data.subcategorias)) {
          data.subcategorias.forEach((subcategoria) => {
            ...
            li.addEventListener("click", function () {
              ...
              if (currentSubcategoryId) {
                const currentLi = listaSubcategorias.querySelector(
                  `li[data-subcategoria-id="${currentSubcategoryId}"]`
                );
                if (currentLi) {
          ...
          console.error(
            "La respuesta no contiene subcategorías válidas:",
            data
          );
        }
      })
  ...
  function loadSubcategoryDetailsAndSchedules(subcategoriaId) {
    ...
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/subcategories/${subcategoriaId}`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      .then((subcategoria) => {
        const detalleNombreInput =
          document.getElementById("detalleNombreInput");
        const detalleVacantesInput = document.getElementById(
          "detalleVacantesInput"
        );

        detalleNombreInput.value = subcategoria.subcategorie.nombre || "";
        detalleNombreInput.dataset.subcategoriaId =
          subcategoria.subcategorie.id;
        detalleVacantesInput.value =
          subcategoria.subcategorie.vacantes !== undefined
            ? subcategoria.subcategorie.vacantes.toString()
            : "0";
  ...
  formCrearSubcategoria.addEventListener("submit", function (event) {
    ...
    fetch("https://backend-laravel-wl09.onrender.com/api/subcategories", {
      method: "POST",
      headers: {
  ...
  function loadVacancies(subcategoriaId) {
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/subcategories/${subcategoriaId}/vacancies`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      .then((data) => {
        ...
        if (totalSpan && disponiblesSpan && ocupadasSpan) {
          totalSpan.textContent =
            data.totalVacantes !== undefined ? data.totalVacantes : 0;
          disponiblesSpan.textContent =
            data.vacantesDisponibles !== undefined
              ? data.vacantesDisponibles
              : 0;
          ocupadasSpan.textContent =
            data.vacantesOcupadas !== undefined ? data.vacantesOcupadas : 0;
        } else {
  ...
  function loadSchedules(subcategoriaId) {
    ...
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/subcategories/${subcategoriaId}/schedules`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        ...
        if (data.horarios && data.horarios.length > 0) {
          data.horarios.forEach((horario) => {
            ...
            nuevoElemento.innerHTML = `
                    ...
                    `;
            const btnEliminar = nuevoElemento.querySelector(
          ...
          listaHorarios.innerHTML =
            "<li>No hay horarios disponibles para esta subcategoría</li>";
        }
      })
      .catch((error) => {
  ...
  function eliminarHorario(horarioId) {
    fetch(
      `https://backend-laravel-wl09.onrender.com/api/schedules/${horarioId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      .then((response) => {
        ...
        const elementoAEliminar = document.querySelector(
          `.horarios-list li[data-horario-id="${horarioId}"]`
        );
  ...
  formCrearHorario.addEventListener("submit", function (event) {
    ...
    const subcategoriaId =
      document.getElementById("detalleNombreInput").dataset.subcategoriaId;
      ...
      fetch(
        ...
        .then((data) => {
          if (data.subcategorie) {
            document.getElementById("detalleNombreInput").value =
              data.subcategorie.nombre || "";
            document.getElementById("detalleVacantesInput").value =
              data.subcategorie.vacantes !== undefined
                ? data.subcategorie.vacantes.toString()
                : "0";

            const categoriaId = data.subcategorie.categoria_id;
            loadSubcategories(categoriaId);

            const formDetallesSubcategoria = document.getElementById(
              "formDetallesSubcategoria"
            );
            formDetallesSubcategoria.reset();
          } else {
            console.error(
              "La respuesta no contiene información de subcategoría válida:",
              data
            );
          }
        })
