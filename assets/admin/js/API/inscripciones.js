// Variables globales
const API_URL = "https://backend-laravel-wl09.onrender.com/api/inscripciones/";
...
function displayInscriptions(inscripciones) {
  ...
  inscripciones.forEach((inscripcion) => {
    const avatarUrl = `https://backend-laravel-wl09.onrender.com/${inscripcion.usuario.imagen_perfil}`;
    ...
    row.innerHTML = `
      ...
      inscripcion.usuario.nombre
            ...
            <td>${formattedDate}</td>
            <td><span class="status ${inscripcion.estado.toLowerCase()}">${
      inscripcion.estado
    }</span></td>
            <td>${inscripcion.numero_vacante}</td>
            <td><i class="ri-edit-line editToggle" data-id="${
              inscripcion.id
            }"></i></td>
        `;
  ...
  fetch(`${API_URL}${inscripcionId}`, {
    ...
    .then((data) => {
      if (data.status === 200) {
        ...
        console.error(
          "Error al obtener detalles de la inscripción:",
          data.message
        );
      }
    })
...
function fillRegistrationDetails(inscripcion) {
  ...
  document.getElementById(
    "username"
  ).innerText = `${inscripcion.usuario.nombre} ${inscripcion.usuario.apellido_paterno}`;
  document.getElementById("user-email").innerText =
    inscripcion.usuario.email || "null@null.com";
  ...
  document.getElementById("date").value = formatDateForInput(
    new Date(inscripcion.created_at)
  );
  ...
  document.getElementById("horarie-regis").value = inscripcion.horario
    ? `${inscripcion.horario.hora_inicio}-${inscripcion.horario.hora_fin}`
    : "";
...
function clearFields() {
  ...
  document.getElementById("status").disabled = true;
  document.getElementById("day-regis").innerHTML =
