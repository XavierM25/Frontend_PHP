document.addEventListener("DOMContentLoaded", function () {
  ...
  const baseUrl = "https://backend-laravel-wl09.onrender.com/api/reportes";

  // Función para limpiar la previsualización
  function clearPreview() {
    previewContainer.innerHTML =
      "<span>No se encontró una previsualización</span>";
  }
      ...
      if (filters.fecha_inicio && filters.fecha_fin) {
        fetch(`${baseUrl}/preview`, {
          ...
          .then((response) => {
            ...
            if (!response.ok) {
              throw new Error(
                "No se pudo obtener la previsualización de datos."
              );
            }
          ...
          .then((data) => {
            if (data && data.data && data.data.length > 0) {
              ...
              switch (filters.modulo) {
                case "Usuarios":
                  previewHTML +=
                    "<th>ID</th><th>Nombre</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Sexo</th><th>Username</th><th>Email</th><th>Fecha Nacimiento</th><th>Campus</th><th>Carrera</th><th>Número de Teléfono</th><th>Fecha Creación</th>";
                  break;
                case "Ranking":
                  previewHTML +=
                    "<th>ID Usuario</th><th>Nombre Usuario</th><th>ID Subcategoría</th><th>Nombre Subcategoría</th><th>Total Puntos</th><th>Estado</th><th>Última Actualización</th>";
                  break;
                case "Actividades":
                  previewHTML +=
                    "<th>ID</th><th>Subcategoría Nombre</th><th>Categoría ID</th><th>Categoría Nombre</th><th>Vacantes</th><th>Creado en</th><th>Actualizado en</th><th>Vacantes Ocupadas</th><th>Horarios</th>";
                  break;
                case "Inscripciones":
                  previewHTML +=
                    "<th>ID</th><th>ID Usuario</th><th>Nombre Usuario</th><th>ID Categoría</th><th>Nombre Categoría</th><th>ID Subcategoría</th><th>Nombre Subcategoría</th><th>ID Horario</th><th>Fecha Horario</th><th>Estado</th><th>Número Vacante</th><th>Comentarios</th><th>Creado en</th><th>Actualizado en</th>";
                  break;
                default:
                  break;
              }
                ...
                switch (filters.modulo) {
                  ...
                  case "Inscripciones":
                    previewHTML += `<td>${item.id}</td>
                                                ...
                                                <td>${
                                                  item.horario
                                                    ? `${item.horario.dia}-${item.horario.hora_inicio}-${item.horario.hora_fin}`
                                                    : ""
                                                }</td>
                                                <td>${item.estado}</td><td>${
                      item.numero_vacante
                    }</td>
                                                <td>${
                                                  item.comentarios
    ...
    fetch(`${baseUrl}`, {
      ...
      .catch((error) => {
        ...
        historySection.innerHTML =
          "<span>Error al cargar el historial de reportes.</span>";
      });
  }
