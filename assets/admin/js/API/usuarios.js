document.addEventListener("DOMContentLoaded", function () {
  const baseUrl = "https://backend-laravel-wl09.onrender.com/api/users";
  ...
  const modalConEliminarUsuario = document.getElementById(
    ...
    users.forEach((user) => {
      ...
      row.innerHTML = `
                ...
                <td class="plusButtons">
                    <div class="editCont">
                        <button class="btnEditarUsuario" data-id="${
                          user.id
    ...
    fetch(`${baseUrl}/${userId}`, {
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          inputImagenPerfil.addEventListener(
            "change",
            handleImagenPerfilChange
          );
        } else {
  ...
  function saveChanges(userId) {
    ...
    fetch(`https://backend-laravel-wl09.onrender.com/api/users/${userId}`, {
      method: "PATCH",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
  ...
  function fillUserDetails(user) {
    ...
    document.getElementById("user-profile-pic").src =
      "https://backend-laravel-wl09.onrender.com/" + user.imagen_perfil;
